var SmoothScroll = function(args){
	...
	this.getThreshold = function(){
		...
		if (deviceDetector.device == "mobile") {
			// _thresholdRatio = 52.5;
			_thresholdRatio = 60;
			this.state.bounds.threshold = this.state.bounds.scrollHeight;
			if (this.state.bounds.threshold == 0) this.state.bounds.threshold = (_thresholdRatio * window.innerWidth) / 100;
		} else {
			_thresholdRatio = 26.04;
			this.state.bounds.threshold = (_thresholdRatio * window.innerWidth) / 100;
		}

		console.log(this.state.bounds.threshold)
	}
